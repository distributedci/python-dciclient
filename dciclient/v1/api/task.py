...
def _get_location(context, id):
    uri = f"{context.dci_cs_api_v2}/tasks/{id}/content"
    response = context.session.get(uri, allow_redirects=False)
    response.raise_for_status()
    location = response.headers.get("Location")
    if not location:
        raise RuntimeError(
            f"No Location header returned by {uri} (status={response.status_code})"
        )
    return location


def content(context, id):
    location = _get_location(context, id)
    get_response = requests.get(location)
