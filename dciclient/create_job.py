...
def get_json(response, key, error_msg):
    ...
    if key not in result_json:
        if "payload" in result_json and "error" in result_json["payload"]:
            print("%s: %s"
                  % (error_msg, result_json["payload"]["error"]))
        else:
            print(error_msg)
        sys.exit(1)
...
def run(context, args):
    ...
    for kv in args.kv:
        ...
        job = get_json(res, "kv", "Error, unable to add key/value (%s/%s)" % (key, value))

    # created_at can only be set after the job has been created
    if args.created_at:
        res = dci_job.update(context, id=job["id"], created_at=args.created_at, etag=job["etag"])
